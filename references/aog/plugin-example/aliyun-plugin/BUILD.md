# Aliyun Plugin - Cross-Platform Build Guide
...
│       ├── text-to-image.go   # Text to image service (non-streaming)
│       ├── image-to-image.go  # Image to image service (non-streaming)
│       ├── speech-to-text.go  # Speech to text service (non-streaming)
│       ├── speech-to-text-ws.go  # Speech to text ws service (streaming)
│       └── text-to-speech.go # Text to speech service (non-streaming)
...
The plugin provides seven services:
...
- **Endpoint**: Special URL (https://nls-gateway-cn-shanghai.aliyuncs.com/stream/v1/asr)
- **Streaming**: ❌ Not supported
- **Protocol**: HTTP

### 6. Speech-to-Text WebSocket Service
- **Endpoint**: WebSocket URL (wss://dashscope.aliyuncs.com/api-ws/v1/inference)
- **Streaming**: ✅ Supported
- **Protocol**: WebSocket
