...
type WebsocketHandler interface {
	HandleWebsocket(ctx context.Context, connID string, authInfo string, inStream <-chan client.BidiMessage, outStream chan<- client.BidiMessage) error
}

// ServiceRequest is the shared request format
type ServiceRequest struct {
	Service string                 `json:"service"`
	Data    map[string]interface{} `json:"data"`
}

// ServiceResponse is the shared response format
type ServiceResponse struct {
	Data  map[string]interface{} `json:"data"`
	Error string                 `json:"error,omitempty"`
}

// ClientInterface abstracts the HTTP client for decoupling and testing
type ClientInterface interface {
	...
	Websocket(ctx context.Context, service string, connID string, authInfo string, inStream <-chan client.BidiMessage, outStream chan<- client.BidiMessage) error
}
