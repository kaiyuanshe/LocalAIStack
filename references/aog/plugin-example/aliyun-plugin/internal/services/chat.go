...
// ChatService handles both streaming and non-streaming chat workflows
type ChatService struct {
	client ClientInterface
}

// NewChatService instantiates a ChatService
...
func (s *ChatService) HandleUnary(ctx context.Context, authInfo string, request []byte) ([]byte, error) {
	...
	if err := s.client.Do(ctx, http.MethodPost, "chat", authInfo, request, &aliyunResp); err != nil {
		return nil, fmt.Errorf("aliyun chat failed: %w", err)
	}
...
func (s *ChatService) HandleStreaming(ctx context.Context, authInfo string, request []byte, ch chan<- client.StreamChunk) {
	...
	for {
		...
		case err := <-errChan:
			if err != nil {
				ch <- client.StreamChunk{Error: fmt.Errorf("aliyun streaming failed: %w", err)}
			}
			return
