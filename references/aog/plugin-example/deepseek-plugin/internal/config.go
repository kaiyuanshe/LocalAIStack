...
import (
	...
	"github.com/intel/aog/plugin/examples/deepseek-plugin/internal/utils"
	"gopkg.in/yaml.v3"
)

// Config holds the Deepseek plugin configuration
type Config struct {
	YamlConfig
...
func loadPluginYamlAndConfig() (*YamlConfig, error) {
	...
	if err != nil {
		// If plugin.yaml is missing, fall back to defaults
