...
func ExpandPath(path string, vars map[string]string) (string, error) {
	...
	// Expand all supplied placeholders
	expanded := path
...
func GetPluginDir() (string, error) {
	...
	// Search upward from executable dir for plugin.yaml (binary may be under bin/<platform>)
	...
	for i := 0; i < 5; i++ { // search up to 5 levels
		...
		if _, err := os.Stat(manifestPath); err == nil {
			// Found plugin.yaml, that's the plugin root
...
// EnsureDir creates directory if it does not exist
...
// PathExists indicates whether the path exists
...
func IsExecutable(path string) bool {
	...
	// On Windows ensure file exists
	if runtime.GOOS == "windows" {
		return !info.IsDir()
	}

	// On Unix ensure executable bits are set
