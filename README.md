...
* Linux/macOS：`./build/las` 和 `./build/las-server`
* Windows PowerShell：`.\build\las.exe` 和 `.\build\las-server.exe`
...
* Linux/macOS：`./build/las`（CLI）、`./build/las-server`（服务端）
* Windows PowerShell：`.\build\las.exe`（CLI）、`.\build\las-server.exe`（服务端）

### 3. 使用 CLI

以下示例默认使用 Linux/macOS 写法；在 Windows PowerShell 中请将 `./build/las` 替换为 `.\build\las.exe`。
